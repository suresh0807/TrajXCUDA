...
////////////////////////////////////LIFETIME-PART///////////////////////////////////////////
	...
	float *Hexch, *Hexch1, *HexchSSP, *HexchSSp1;
...
//######################################################################################################################################### 
    ...
    {
      if(type==0) {HB_lifestyle="continuous"; cout<<"Continuous lifetime"<<endl;}
      else if(type==1) {HB_lifestyle="transient";transtime=2; cout<<"Transient lifetime"<<endl;}
      else if(type==2){HB_lifestyle="intermittent";cout<<"Intermittent lifetime"<<endl;}
...
initHBcol1 = (int **) malloc (sizeof(int *)*SD_store*2);
	...
	if( Hbondxyz[j1*9+i1*nElt1*9+4+i*SD_store*nElt1*9] !=0 && exch1[j1+i1*nElt1+i*SD_store*nElt1] == whichwater ) 
        {
	  Hexch1[j1+Hbondneigh[j1*OHsamples+i1*nElt1*OHsamples+1+i*SD_store*nElt1*OHsamples]*nElt1+i1*nElt2*nElt1]=1.0; //+1 here is for acceptor check
	  initHBcol1[i1][chker] = (j1+Hbondneigh[j1*OHsamples+i1*nElt1*OHsamples+1+i*SD_store*nElt1*OHsamples]*nElt1); chker+=1;
	}
	if( Hbondxyz[j1*9+i1*nElt1*9+4+i*SD_store*nElt1*9] !=0 ) 
        {
	  Hexch[j1+Hbondneigh[j1*OHsamples+i1*nElt1*OHsamples+1+i*SD_store*nElt1*OHsamples]*nElt1+i1*nElt2*nElt1]=1.0; //acceptor check
	}
	...
	if(Hbondxyz[j1*9+i1*nElt1*9+4+i*SD_store*nElt1*9] !=0) 
        {
	  Hexch1[j1+Hbondneigh[j1*OHsamples+i1*nElt1*OHsamples+1+i*SD_store*nElt1*OHsamples]*nElt1+i1*nElt2*nElt1]=1.0;//+1 in Hbondneigh is the acceptor
	  Hexch[j1+Hbondneigh[j1*OHsamples+i1*nElt1*OHsamples+1+i*SD_store*nElt1*OHsamples]*nElt1+i1*nElt2*nElt1]=1.0;
	  initHBcol1[i1][chker] = (j1+Hbondneigh[j1*OHsamples+i1*nElt1*OHsamples+1+i*SD_store*nElt1*OHsamples]*nElt1); chker+=1;
	}
       }
     }  
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////
     ...
     {      
       ...
       Hexch_sized1 = (float *) malloc (sizeof(float)*initHBnum[j+i*SD_store]*SD_store);
	     ...
	     //cout<<ja<<" "<<initHBcol[j+i*SD_store][ja]<<endl;
	       Hexch_sized1[ja+ia*initHBnum[j+i*SD_store]] = Hexch1[initHBcol1[j][ja]+j*nElt2*nElt1+ia*nElt2*nElt1];// origin samples
	       Hexch_sized[ja+ia*initHBnum[j+i*SD_store]]  =  Hexch[initHBcol1[j][ja]+j*nElt2*nElt1+ia*nElt2*nElt1];
	 ...
	 if(j==1 && i==0)
	 {
	   ...
	   for(int ia=0;ia<SD_store;ia++)
           {
           for(int ja=0; ja<initHBnum[j+i*SD_store]; ja++)////only those present in the first frame in the restart bin
           { 
	       cout<<Hexch_sized[ja+ia*initHBnum[j+i*SD_store]] <<" ";
	   }
	   cout<<endl;
	   }
	   cout<<"after: "<<endl;
	 ...
	 */
	 ...
	 {
         for(int ja=0; ja<initHBnum[j+i*SD_store]; ja++)///for continuous lifetime
         {
           for(int ia=1;ia<SD_store;ia++)
           {
	     if(Hexch_sized[ja+(ia-1)*initHBnum[j+i*SD_store]] == 0) {Hexch_sized[ja+ia*initHBnum[j+i*SD_store]] = 0;} 
	   }
	 }
	 }
	 ...
	 if(HB_lifestyle=="transient")
	 {
	   //do transient time approximation here
         for(int ja=0; ja<initHBnum[j+i*SD_store]; ja++)///for transient intermittent lifetime
         {
           for(int ia=0;ia<SD_store;ia++)
           {
		 ...
		 else if(Hexch_sized[ja+(ia+chk)*initHBnum[j+i*SD_store]] == 1){ Hexch_sized[ja+ia*initHBnum[j+i*SD_store]]=1;break;}
	       }
	    } 
	   }
	 }
	 ...
	 */
	 ...
	 for(int ja=0; ja<initHBnum[j+i*SD_store]; ja++)
	 {
      ...
      {
      for(int j1=0; j1<nElt2; j1++)//acceptor
      {
	for(int k1=0; k1<nElt1; k1++)//shared hydrogen
	{
	  Hbondnum[i1+i*SD_store]+=Hexch1[k1+j1*nElt1+i1*nElt2*nElt1];
	}
      }
      }
   // for(int ja =1; ja<SD_store-1;ja++)
...
*/
	...
	Hexchplot <<"Average number of Hbonds per structure = "<<avg_Hbondnum<<endl;
      ...
      if(type==0) {system("mv ct.data ct-continuous.data; mv ft.data ft-continuous.data; mv ft-avg.data ft-avg-continuous.data; \
	mv ct-integrate.data ct-integrate-continuous.data; mv Hbond-lifetime.data Hbond-lifetime-continuous.data; mv Hbondnum.data Hbondnum-continuous.data ");}
      else if(type==1) {system("mv ct.data ct-transient.data; mv ft.data ft-transient.data; mv ft-avg.data ft-avg-transient.data; \
	mv ct-integrate.data ct-integrate-transient.data; mv Hbond-lifetime.data Hbond-lifetime-transient.data; mv Hbondnum.data Hbondnum-transient.data ");}
      else if(type==2){system("mv ct.data ct-intermittent.data; mv ft.data ft-intermittent.data; mv ft-avg.data ft-avg-intermittent.data; \
	mv ct-integrate.data ct-integrate-intermittent.data; mv Hbond-lifetime.data Hbond-lifetime-intermittent.data; mv Hbondnum.data Hbondnum-intermittent.data ");}
    }
  free(Hbondnum);
  free(Hexch);
  free(Hexch1);
