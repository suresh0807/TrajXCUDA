...
for(int i=0;i<nstruct/skip;i++)
  ...
  float normnum;
      ...
      else if(HB_for=="all"){normnum = nElt1;} 
...
//storing the coordinates of the Hbonds in nElt2*nElt1*nstruct/skip matrix
...
////////////////////////////////////LIFETIME-PART///////////////////////////////////////////
	...
	float *Hexch, *Hexch1, *HexchSSP, *HexchSSP1, *HneighSSP, *HdistSSP, *HangSSP, *HdistOOSSP, *HneighallSSP;
    //Hexch = (float *) malloc (sizeof(float)*nElt2*nElt1*SD_store*2);
    //Hexch1 = (float *) malloc (sizeof(float)*nElt2*nElt1*SD_store*2);
    HexchSSP = (float *) malloc (sizeof(float)*nElt1*SD_store*2);
    HneighSSP = (float *) malloc (sizeof(float)*nElt1*SD_store*2);
    //HdistSSP = (float *) malloc (sizeof(float)*nElt1*SD_store*2);// for ssp second matrix
    HangSSP = (float *) malloc (sizeof(float)*nElt1*SD_store*2*8);
    HdistOOSSP = (float *) malloc (sizeof(float)*nElt1*SD_store*2*8);
    HneighallSSP = (float *) malloc (sizeof(float)*nElt1*SD_store*2*8);
    HexchSSP1 = (float *) malloc (sizeof(float)*nElt1*SD_store*2);
 //########################################################################################################################################
//########################################################################################################################################
...
//######################################################################################################################################### 
	 if(HB_lifestyle=="SSP")
	 {
	   cout<<"SSP life time scheme will be used"<<endl;
	 }
	  if(HB_lifestyle=="switchbond")
	 {
	   cout<<"switchbond life time scheme will be used"<<endl;
	 }
	 int samples=8;
    ...
    /*
         ...
         {
           for(int j1=0; j1<nElt2; j1++)//acceptor oxygen
           {
	     for(int k1=0; k1<nElt1; k1++)//shared hydrogen
             {
	       Hexch[k1+j1*nElt1+i1*nElt2*nElt1]=0.0;
	       Hexch1[k1+j1*nElt1+i1*nElt2*nElt1]=0.0;
	     }
	   }
	 }
 */ 
 
	 if(HB_lifestyle=="SSP"|| HB_lifestyle=="switchbond")
	 {
	   for(int i1=0;i1<SD_store*2;i1++)
           {
	     for(int k1=0; k1<nElt1; k1++)//shared hydrogen
             {
	       HexchSSP[k1+i1*nElt1]=-1.0;
	       HexchSSP1[k1+i1*nElt1]=0.0;
	       HneighSSP[k1+i1*nElt1]=0.0;
	       //HdistSSP[k1+i1*nElt1]=0.0;
	       for(int samp=0; samp<8; samp++)
	       {
	       HdistOOSSP[samp+k1*samples+i1*samples*nElt1]=0.0;
	       HangSSP[samp+k1*samples+i1*samples*nElt1]=0.0;
	       HneighallSSP[samp+k1*samples+i1*samples*nElt1]=0.0;
	       }
	     }
	   }
	 }
...
initHBcol1 = (int **) malloc (sizeof(int *)*SD_store*2);
     ...
     {
     initHBcol1[i1] = (int *) malloc (sizeof(int)*initHBnum[i1+i*SD_store]);
     for(int j1=0; j1<initHBnum[i1+i*SD_store]; j1++)// for number of H bonds this frame
     {
    ...
    for(int i1=0;i1<SD_store*2;i1++)
     {int chker=0;
      for(int j1=0; j1<nElt1; j1++)
       {
	if( Hbondxyz[j1*9+i1*nElt1*9+4+i*SD_store*nElt1*9] !=0 && exch1[j1+i1*nElt1+i*SD_store*nElt1] == whichwater ) 
        {
	 // Hexch1[j1+int(Hbondxyz[j1*9+i1*nElt1*9+5+i*SD_store*nElt1*9])*nElt1+i1*nElt2*nElt1]=1.0; //+4 here is for acceptor check
	  initHBcol1[i1][chker] = (j1+int(Hbondxyz[j1*9+i1*nElt1*9+5+i*SD_store*nElt1*9])*nElt1); chker+=1;
	  HexchSSP1[j1+i1*nElt1] = 1.0;
	}
	if( Hbondxyz[j1*9+i1*nElt1*9+4+i*SD_store*nElt1*9] !=0 ) 
        {
	  //Hbondxyz[j*9+i*nElt1*9+5]
	  ///Hexch[j1+int(Hbondxyz[j1*9+i1*nElt1*9+5+i*SD_store*nElt1*9])*nElt1+i1*nElt2*nElt1]=1.0; //acceptor check
	  HexchSSP[j1+i1*nElt1] = int(Hbondxyz[j1*9+i1*nElt1*9+5+i*SD_store*nElt1*9]); //the id is copied
	}
	 //HdistSSP[j1+i1*nElt1] = Hbondneighdist[j1*OHsamples+i1*nElt1*OHsamples+1+i*SD_store*nElt1*OHsamples];
	 HneighSSP[j1+i1*nElt1] = Hbondneigh[j1*OHsamples+i1*nElt1*OHsamples+1+i*SD_store*nElt1*OHsamples];
	 //HdistSSP[j1+i1*nElt1] = Hbondneighdist[j1*OHsamples+i1*nElt1*OHsamples+1+i*SD_store*nElt1*OHsamples];
	 for(int samp=0; samp<8; samp++)
	       {
	 HdistOOSSP[samp+j1*samples+i1*samples*nElt1] = distOOsamples[samp+j1*OHsamples+i1*nElt1*OHsamples+1+i*SD_store*nElt1*OHsamples];
	 HangSSP[samp+j1*samples+i1*samples*nElt1] = anglesHOOsamples[samp+j1*OHsamples+i1*nElt1*OHsamples+1+i*SD_store*nElt1*OHsamples];
	 HneighallSSP[samp+j1*samples+i1*samples*nElt1] = Hbondneigh[samp+j1*OHsamples+i1*nElt1*OHsamples+1+i*SD_store*nElt1*OHsamples];
	       }
      }
     }
    }
	...
	if(Hbondxyz[j1*9+i1*nElt1*9+4+i*SD_store*nElt1*9] !=0) 
        {
	 // Hexch1[j1+int(Hbondxyz[j1*9+i1*nElt1*9+5+i*SD_store*nElt1*9])*nElt1+i1*nElt2*nElt1]=1.0;//+1 in Hbondneigh is the acceptor
	 // Hexch[j1+int(Hbondxyz[j1*9+i1*nElt1*9+5+i*SD_store*nElt1*9])*nElt1+i1*nElt2*nElt1]=1.0;
	  HexchSSP1[j1+i1*nElt1] = 1.0;
	  HexchSSP[j1+i1*nElt1] = int(Hbondxyz[j1*9+i1*nElt1*9+5+i*SD_store*nElt1*9]);
	  initHBcol1[i1][chker] = (j1+int(Hbondxyz[j1*9+i1*nElt1*9+5+i*SD_store*nElt1*9])*nElt1); chker+=1;
	}
	HneighSSP[j1+i1*nElt1] = Hbondneigh[j1*OHsamples+i1*nElt1*OHsamples+1+i*SD_store*nElt1*OHsamples];
	//HdistSSP[j1+i1*nElt1] = Hbondneighdist[j1*OHsamples+i1*nElt1*OHsamples+1+i*SD_store*nElt1*OHsamples];
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////
     ...
     {      
       ...
       float *Hneigh_sized;//for ssp
       //float *Hdist_sized;//for ssp
       float *HdistOO_sized;//for ssp
       float *Hang_sized;//for ssp
       float *Hneighall_sized;//for ssp
       Hexch_sized = (float *) malloc (sizeof(float)*nElt1*SD_store);
       Hexch_sized1 = (float *) malloc (sizeof(float)*nElt1*SD_store);
       Hneigh_sized = (float *) malloc (sizeof(float)*nElt1*SD_store);
       //Hdist_sized = (float *) malloc (sizeof(float)*nElt1*SD_store);
       HdistOO_sized = (float *) malloc (sizeof(float)*nElt1*SD_store*8);
       Hang_sized = (float *) malloc (sizeof(float)*nElt1*SD_store*8);
       Hneighall_sized = (float *) malloc (sizeof(float)*nElt1*SD_store*8);
         for(int ia=0;ia<SD_store;ia++)
         {
           for(int ja=0; ja<nElt1; ja++)////only those H that are hydrogen bonded in the first frame in the restart bin
           { //if(i==0) cout << initHBcol[j]<<endl;
	     //cout<<ja<<" "<<initHBcol[j+i*SD_store][ja]<<endl;
	       Hexch_sized1[ja+ia*nElt1] = HexchSSP1[ja+ia*nElt1+j*nElt1];// origin samples
	       Hexch_sized[ja+ia*nElt1]  = HexchSSP[ja+ia*nElt1+j*nElt1];
	       Hneigh_sized[ja+ia*nElt1]  = HneighSSP[ja+ia*nElt1+j*nElt1];
	       //Hdist_sized[ja+ia*nElt1]  = HdistSSP[ja+ia*nElt1+j*nElt1];
	       for(int samp=0; samp<8; samp++)
	       {
	       HdistOO_sized[samp+ja*samples+ia*samples*nElt1]  = HdistOOSSP[samp+ja*samples+ia*nElt1*samples+j*samples*nElt1];
	       Hang_sized[samp+ja*samples+ia*samples*nElt1]  = HangSSP[samp+ja*samples+ia*samples*nElt1+j*samples*nElt1];
	       Hneighall_sized[samp+ja*samples+ia*samples*nElt1]  = HneighallSSP[samp+ja*samples+ia*samples*nElt1+j*samples*nElt1];
	       }
	       //Hexch_sized1[ja+ia*initHBnum[j+i*SD_store]] = Hexch1[initHBcol1[j][ja]+ia*nElt2*nElt1];// origin samples
	       //Hexch_sized[ja+ia*initHBnum[j+i*SD_store]]  =  Hexch[initHBcol1[j][ja]+ia*nElt2*nElt1];
	   }
	 }
	 /*
	 if(j==0 && i==0)
	 {
	   ...
	   for(int ia=0;ia<SD_store;ia++)
           {
           //for(int ja=0; ja<nElt1; ja++)////only those present in the first frame in the restart bin
	     for(int ja=5; ja<7; ja++)
           { 
	     cout.width(10);
	       cout<<Hexch_sized[ja+ia*nElt1] <<" ";
	       cout.width(10);
	       cout<<Hneigh_sized[ja+ia*nElt1]<<" ";
	       //cout.width(10);
	      //cout<<Hdist_sized[ja+ia*nElt1]<<" ";
	       cout.width(10);
	       cout<<Hang_sized[ja*samples+ia*samples*nElt1]*(180.0/3.14159)<<" ";
	       cout.width(10);
	       cout<<HdistOO_sized[ja*samples+ia*samples*nElt1]<<" ";
	   }
	   cout<<endl;
	   }
	   cout<<"after: "<<endl;
	 ...
	 */
	 if(HB_lifestyle=="switchbond")
	 {
	   for(int k1=0; k1<nElt1; k1++)//shared hydrogen
             {int chkr=0;
	      for(int i1=0;i1<SD_store;i1++)
               {
	       if(Hexch_sized[k1+i1*nElt1]==-1.0 && chkr==0) { Hexch_sized[k1+i1*nElt1]=0;}
	       else if(Hexch_sized[k1+i1*nElt1]!=-1.0 && chkr==0) {chkr=Hexch_sized[k1+i1*nElt1];Hexch_sized[k1+i1*nElt1]=1;}
	       else if(Hexch_sized[k1+i1*nElt1]!=-1.0 && chkr==Hexch_sized[k1+i1*nElt1]){Hexch_sized[k1+i1*nElt1]=1;}
	       else if(Hexch_sized[k1+i1*nElt1]==-1.0 && chkr!=0 && chkr!=-1){Hexch_sized[k1+i1*nElt1]=1;}
	       else if(Hexch_sized[k1+i1*nElt1]==-1.0 && chkr==-1){Hexch_sized[k1+i1*nElt1]=0;}
	       else if(Hexch_sized[k1+i1*nElt1]!=-1.0 && chkr!=Hexch_sized[k1+i1*nElt1]){Hexch_sized[k1+i1*nElt1]=0;chkr=-1;}
	     }
	   }
	 }
	 ...
	 if(HB_lifestyle=="SSPA")
	 {
	   for(int k1=0; k1<nElt1; k1++)//shared hydrogen
	      ...
	      for(int i1=0;i1<SD_store;i1++)
               {
	       if(Hexch_sized[k1+i1*nElt1]==-1.0 && chkr==0) { Hexch_sized[k1+i1*nElt1]=0;}
	       else if(Hexch_sized[k1+i1*nElt1]!=-1.0 && chkr==0) {chkr=Hexch_sized[k1+i1*nElt1];Hexch_sized[k1+i1*nElt1]=0;}
	       else if(Hexch_sized[k1+i1*nElt1]!=-1.0 && chkr==Hexch_sized[k1+i1*nElt1]){Hexch_sized[k1+i1*nElt1]=0;}
	       else if(Hexch_sized[k1+i1*nElt1]==-1.0 && chkr!=0 && chkr!=-1){Hexch_sized[k1+i1*nElt1]=0;}
	       else if(chkr==-1){Hexch_sized[k1+i1*nElt1]=1;}
	       else if(Hexch_sized[k1+i1*nElt1]!=-1.0 && chkr!=Hexch_sized[k1+i1*nElt1]){Hexch_sized[k1+i1*nElt1]=1;chkr=-1;}
	     }
	   }
	 }
	 //Switch bond definition of matti with strict criterium
	 if(HB_lifestyle=="SSP-switchbond")//If a new acceptor oxygen candidate becomes the closest neighbour, the Hydrogen the bond is broken //
	 {
	   for(int k1=0; k1<nElt1; k1++)//shared hydrogen
             {int chkr=0;
	      for(int i1=0;i1<SD_store;i1++)
	       ...
	       else if(Hexch_sized[k1+i1*nElt1]==-1.0 && chkr!=0 && chkr!=-1 && chkr==Hneigh_sized[k1+i1*nElt1]){Hexch_sized[k1+i1*nElt1]=0;}
	       else if(chkr==-1){Hexch_sized[k1+i1*nElt1]=1;}
	       //else if(Hexch_sized[k1+i1*nElt1]==-1.0 && chkr!=0 && chkr!=-1 && chkr==Hneigh_sized[k1+i1*nElt1]&&Hdist_sized[k1+i1*nElt1]>2.5){Hexch_sized[k1+i1*nElt1]=0;}
	       else if(Hexch_sized[k1+i1*nElt1]==-1.0 && chkr!=0 && chkr!=-1 && chkr!=Hneigh_sized[k1+i1*nElt1]){Hexch_sized[k1+i1*nElt1]=1;chkr=-1;}
	       else if(Hexch_sized[k1+i1*nElt1]!=-1.0 && chkr!=Hexch_sized[k1+i1*nElt1]){Hexch_sized[k1+i1*nElt1]=1;chkr=-1;}
	     }
	   }
	 }
	// cout<<"I am fine"<<endl;
	 if(HB_lifestyle=="SSPAA")
	 {
	   for(int k1=0; k1<nElt1; k1++)//shared hydrogen
             {int chkr=0;
	      for(int i1=0;i1<SD_store;i1++)
	       ...
	       else if(Hexch_sized[k1+i1*nElt1]==-1.0 && chkr!=0 && chkr!=-1 )
	       {
		 for(int samp=1; samp<8; samp++)
	          {
		   if (HdistOO_sized[samp+k1*samples+i1*samples*nElt1] <3.8 && HdistOO_sized[samp+k1*samples+i1*samples*nElt1] !=0 && \
		     Hang_sized[samp+k1*samples+i1*samples*nElt1] < 0.698132 && Hang_sized[samp+k1*samples+i1*samples*nElt1] !=0)
		   {Hexch_sized[k1+i1*nElt1]=1;chkr=-1;break;}
		   else
		   {Hexch_sized[k1+i1*nElt1]=0; }
		  }
	       }
	       //else if(Hexch_sized[k1+i1*nElt1]==-1.0 && chkr!=0 && chkr!=-1 && chkr==Hneigh_sized[k1+i1*nElt1] && Hdist_sized[k1+i1*nElt1] <2.5){Hexch_sized[k1+i1*nElt1]=0;}
	       else if(chkr==-1){Hexch_sized[k1+i1*nElt1]=1;}
	  ...
	  if(HB_lifestyle=="SSP")
	 {
	   for(int k1=0; k1<nElt1; k1++)//shared hydrogen
	      ...
	      for(int i1=0;i1<SD_store;i1++)
	       ...
	       else if(Hexch_sized[k1+i1*nElt1]==-1.0 && chkr!=0 && chkr!=-1 && chkr==Hneigh_sized[k1+i1*nElt1])
	       {
		 for(int samp=0; samp<1; samp++)
	          {
		   if (HdistOO_sized[samp+k1*samples+i1*samples*nElt1] >3.8 || Hang_sized[samp+k1*samples+i1*samples*nElt1] > 0.698132)
		   {Hexch_sized[k1+i1*nElt1]=1;chkr=-1;break;}
		   else
		   {Hexch_sized[k1+i1*nElt1]=0; }
		  }
		 ...
		 for(int samp=0; samp<8; samp++)
	          {
		   if (chkr==Hneighall_sized[samp+k1*samples+i1*samples*nElt1])
		   {
		     if(HdistOO_sized[samp+k1*samples+i1*samples*nElt1] > 3.8 || Hang_sized[samp+k1*samples+i1*samples*nElt1] > 0.698132)
		      {
			Hexch_sized[k1+i1*nElt1]=1;chkr=-1;break;
	...
	*/
       ...
       SD1=(float *) malloc (sizeof(float)*nElt1*SD_store);
        for(int ia=0;ia<SD_store;ia++)
        {
	 for(int ja=0; ja<nElt1; ja++)
	 {
      ...
      dim3 dimGrida((nElt1+dimBlocka.x-1)/dimBlocka.x,1,(SD_store+dimBlocka.z-1)/dimBlocka.z);
      ...
      cudaMalloc((void **)&dev_A,sizeof(float)*nElt1*SD_store);
      cudaMalloc((void **)&dev_A1,sizeof(float)*nElt1*SD_store);
      cudaMalloc((void **)&dev_SD1,sizeof(float)*nElt1*SD_store);
      
      cudaMemcpy(dev_A,Hexch_sized1,sizeof(float)*nElt1*SD_store,cudaMemcpyHostToDevice);
      cudaMemcpy(dev_A1,Hexch_sized,sizeof(float)*nElt1*SD_store,cudaMemcpyHostToDevice);
      cudaMemcpy(dev_SD1,SD1,sizeof(float)*nElt1*SD_store,cudaMemcpyHostToDevice);
   
      HBAF_calc<<<dimGrida,dimBlocka>>>(dev_A,dev_A1,dev_SD1,SD_store,nElt1,i,j,origins,skips);
      cudaMemcpy(SD1,dev_SD1,sizeof(float)*nElt1*SD_store,cudaMemcpyDeviceToHost);
      cudaFree(dev_A);
      cudaFree(dev_A1);
      ...
      SDreduce<<<dimGrids,dimBlocks>>>(dev_SD1,dev_SDavg1,SD_store,nElt1,fairy);
      ...
      //cout<<initHBnum[j+i*SD_store]<<" "<<SDavg1[0]<<" ";
	...
	for(int k1=0; k1<nElt1; k1++)//shared hydrogen
	{
	  Hbondnum[i1+i*SD_store]+=HexchSSP1[k1+i1*nElt1];
	}
      }
   // for(int ja =1; ja<SD_store-1;ja++)
   //   {
  ...
  free(HangSSP);
     ...
     // SDsum1[0]=1.0;
      ...
      float SSPdivide=SDsum1[SD_store-1]/(num_bins*restarts);
     for(int ja =0; ja<SD_store;ja++)
     {
       //SDsum[ja]/=float(num_bins*restarts);
       SDsum1[ja]/=float(num_bins*restarts);
       if(HB_lifestyle=="SSP"){SDsum1[ja]=1-(SDsum1[ja]/SSPdivide); if(SDsum1[ja] < 0){SDsum1[ja]=0;} Hexchplotac <<ja*timestep/1000<<" "<<SDsum1[ja]<<endl;}
       else {Hexchplotac <<ja*timestep/1000<<" "<<SDsum1[ja]/SDsum1[0]<<endl;}
