...
/*
//slow way to get the positions of the oxygen atoms found by GPU
   ...
   for(int i=0;i<(nstruct/skip);i++)
   {
	...
	//if(Onum[j+i*nElt1] >1) 
      ...
      if(cell_type == "orthorhombic")
      {
       for(int k =0 ;k<3;k++)
       {
	Hbondxyz[j*5+i*nElt1*5+k] = A[j*3+i*nElt1*3+k];
	vec1[k] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4]*3+k];
	vec2[k] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4+1]*3+k];
	vec3[k] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4+2]*3+k];
	vec4[k] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4+3]*3+k];
	if(k==0) 
	{
	  ...
	  if(fabs(A[j*3+i*nElt1*3+k]-vec3[k]) > xvec/2) 
	          {if (A[j*3+i*nElt1*3+k]-vec3[k] > 0) {vec3[k] = vec3[k] +xvec;}
		   else {vec3[k] = vec3[k] -xvec;}}
	  if(fabs(A[j*3+i*nElt1*3+k]-vec4[k]) > xvec/2) 
	          {if (A[j*3+i*nElt1*3+k]-vec4[k] > 0) {vec4[k] = vec4[k] +xvec;}
		   else {vec4[k] = vec4[k] -xvec;}}	   
	}
	...
	else if(k==1) 
	{
	  ...
	  if(fabs(A[j*3+i*nElt1*3+k]-vec2[k]) > yvec/2) 
          ...
          if(fabs(A[j*3+i*nElt1*3+k]-vec3[k]) > yvec/2) 
	          {if (A[j*3+i*nElt1*3+k]-vec3[k] > 0) {vec3[k] = vec3[k] +yvec;}
		   else {vec3[k] = vec3[k] -yvec;}}
	  if(fabs(A[j*3+i*nElt1*3+k]-vec4[k]) > yvec/2) 
	          {if (A[j*3+i*nElt1*3+k]-vec4[k] > 0) {vec4[k] = vec4[k] +yvec;}
		   else {vec4[k] = vec4[k] -yvec;}}   
	}
	...
	else if(k==2) 
	{
	  ...
	  if(fabs(A[j*3+i*nElt1*3+k]-vec3[k]) > zvec/2) 
	          {if (A[j*3+i*nElt1*3+k]-vec3[k] > 0) {vec3[k] = vec3[k] +zvec;}
		   else {vec3[k] = vec3[k] -zvec;}}	
	  if(fabs(A[j*3+i*nElt1*3+k]-vec4[k]) > zvec/2) 
	          {if (A[j*3+i*nElt1*3+k]-vec4[k] > 0) {vec4[k] = vec4[k] +zvec;}
		   else {vec4[k] = vec4[k] -zvec;}}	   
	}
       ...
       {
	int k=0;
	Hbondxyz[j*5+i*nElt1*5] = A[j*3+i*nElt1*3];
	Hbondxyz[j*5+i*nElt1*5+1] = A[j*3+i*nElt1*3+1];
	Hbondxyz[j*5+i*nElt1*5+2] = A[j*3+i*nElt1*3+2];
	vec1[0] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4]*3];
	vec2[0] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4+1]*3];
	vec3[0] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4+2]*3];
	vec4[0] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4+3]*3];
	vec1[1] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4]*3+1];
	vec2[1] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4+1]*3+1];
	vec3[1] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4+2]*3+1];
	vec4[1] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4+3]*3+1];
	vec1[2] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4]*3+2];
	vec2[2] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4+1]*3+2];
	vec3[2] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4+2]*3+2];
	vec4[2] = B[i*nElt2*3+Hbondneigh[j*4+i*nElt1*4+3]*3+2];
	k=1;
	if(fabs(A[j*3+i*nElt1*3+k]-vec1[k]) > yvec/2) 
	...
	if(fabs(A[j*3+i*nElt1*3+k]-vec3[k]) > yvec/2) 
	          {if (A[j*3+i*nElt1*3+k]-vec3[k] > 0) {vec3[k] = vec3[k] +yvec; vec3[0]=vec3[0]+yxvec;}
		   else {vec3[k] = vec3[k] -yvec; vec3[0]=vec3[0]-yxvec;}}
	if(fabs(A[j*3+i*nElt1*3+k]-vec4[k]) > yvec/2) 
	          {if (A[j*3+i*nElt1*3+k]-vec4[k] > 0) {vec4[k] = vec4[k] +yvec; vec4[0]=vec4[0]+yxvec;}
		   else {vec4[k] = vec4[k] -yvec; vec4[0]=vec4[0]-yxvec;}}	   
	k=0;
	if(fabs(A[j*3+i*nElt1*3+k]-vec1[k]) > xvec/2) 
	          {if (A[j*3+i*nElt1*3+k]-vec1[k] > 0) {vec1[k] = vec1[k] +xvec;}
		   else {vec1[k] = vec1[k] -xvec;}}
	...
	Hbondneighdist[j*4+i*nElt1*4] = sqrt(square(vec1[0]-A[j*3+i*nElt1*3])+square(vec1[1]-A[j*3+i*nElt1*3+1])+square(vec1[2]-A[j*3+i*nElt1*3+2]));
	Hbondneighdist[j*4+i*nElt1*4+1] = sqrt(square(vec2[0]-A[j*3+i*nElt1*3])+square(vec2[1]-A[j*3+i*nElt1*3+1])+square(vec2[2]-A[j*3+i*nElt1*3+2]));
	Hbondneighdist[j*4+i*nElt1*4+2] = sqrt(square(vec3[0]-A[j*3+i*nElt1*3])+square(vec3[1]-A[j*3+i*nElt1*3+1])+square(vec3[2]-A[j*3+i*nElt1*3+2]));
	Hbondneighdist[j*4+i*nElt1*4+3] = sqrt(square(vec4[0]-A[j*3+i*nElt1*3])+square(vec4[1]-A[j*3+i*nElt1*3+1])+square(vec4[2]-A[j*3+i*nElt1*3+2]));
...
ofstream Hneighbf;
Hneighbf.open("Hneighbf.data");

  for(int i=0;i<nstruct/skip;i++)
  {      
    for(int j=0; j<nElt1; j++)
    {    
   //   Hneigh<<j<<" "<<Onum[j+i*nElt1]<<" "<<Hbondneigh[j*4+i*nElt1*4]<<" "<<Hbondneighdist[j*4+i*nElt1*4]<<" "<<Hbondneigh[j*4+i*nElt1*4+1]<<" "<<Hbondneighdist[j*4+i*nElt1*4+1]<<" "<< Hbondxyz[j*5+i*nElt1*5+3]*(180.0/3.14159)<<" "<< Hbondxyz[j*5+i*nElt1*5+4]<<endl;
      Hneighbf<<j<<" "<<Onum[j+i*nElt1]<<" "<<Hbondneigh[j*4+i*nElt1*4]<<" "<<Hbondneighdist[j*4+i*nElt1*4]<<" "<<Hbondneigh[j*4+i*nElt1*4+1]<<" "<<Hbondneighdist[j*4+i*nElt1*4+1]<<" "<<Hbondneigh[j*4+i*nElt1*4+2]<<" "<<Hbondneighdist[j*4+i*nElt1*4+2]<<" "<<Hbondneigh[j*4+i*nElt1*4+3]<<" "<<Hbondneighdist[j*4+i*nElt1*4+3]<<endl;
...
angHoo= (float *) malloc (sizeof(float)*nElt1*(nstruct/skip));

  for(int i=0;i<nstruct/skip;i++)
  {
    for(int j=0; j<nElt1; j++)
    {
      angHoo[j+i*nElt1]=0.0;
    }
  }
	...
	vec2[k] = B[i*nElt2*3+Hbondneigh[j*OHsamples+i*nElt1*OHsamples+1]*3+k];
	if(k==0) 
	{
	  if(fabs(A[j*3+i*nElt1*3+k]-vec1[k]) > xvec/2) 
	          {if (A[j*3+i*nElt1*3+k]-vec1[k] > 0) {vec1[k] = vec1[k] +xvec;}
	...
	Hbondxyz[j*5+i*nElt1*5+1] = A[j*3+i*nElt1*3+1];
	Hbondxyz[j*5+i*nElt1*5+2] = A[j*3+i*nElt1*3+2];
	vec1[0] = B[i*nElt2*3+Hbondneigh[j*OHsamples+i*nElt1*OHsamples]*3];
	vec2[0] = B[i*nElt2*3+Hbondneigh[j*OHsamples+i*nElt1*OHsamples+1]*3];
	vec1[1] = B[i*nElt2*3+Hbondneigh[j*OHsamples+i*nElt1*OHsamples]*3+1];
	vec2[1] = B[i*nElt2*3+Hbondneigh[j*OHsamples+i*nElt1*OHsamples+1]*3+1];
	vec1[2] = B[i*nElt2*3+Hbondneigh[j*OHsamples+i*nElt1*OHsamples]*3+2];
	vec2[2] = B[i*nElt2*3+Hbondneigh[j*OHsamples+i*nElt1*OHsamples+1]*3+2];
	k=1;
	if(fabs(A[j*3+i*nElt1*3+k]-vec1[k]) > yvec/2) 
	          {if (A[j*3+i*nElt1*3+k]-vec1[k] > 0) {vec1[k] = vec1[k] +yvec; vec1[0]=vec1[0]+yxvec;}
		   else {vec1[k] = vec1[k] -yvec; vec1[0]=vec1[0]-yxvec;}}
	...
	//angle between H-acceptor and acceptor-donor order - central atom and other two
	Hbondxyz[j*5+i*nElt1*5+3]=angle(A[j*3+i*nElt1*3],A[j*3+i*nElt1*3+1],A[j*3+i*nElt1*3+2],vec1[0], vec1[1], vec1[2], vec2[0], vec2[1], vec2[2]);
	angHoo[j+i*nElt1]=angle(vec2[0], vec2[1], vec2[2],A[j*3+i*nElt1*3],A[j*3+i*nElt1*3+1],A[j*3+i*nElt1*3+2],vec1[0], vec1[1], vec1[2]);
	Hbondxyz[j*5+i*nElt1*5+4]=sqrt(square(vec2[0]-vec1[0])+square(vec2[1]-vec1[1])+square(vec2[2]-vec1[2]));
	
       //}
     }
     }
 cout <<"all dist,angles, neighbours evaluated"<<endl;   
...
cout<<"Mapping Hbonds to the donor acceptopr matrix for lifetime calculatiuon"<<endl;
...
//Hexch stores the donor acceptor matrix - for water pair survival lifetime
...
if(HBlifetime == "yes")
   {
    ...
    Hexch = (float *) malloc (sizeof(float)*nElt2*nElt2*(nstruct/skip));
    Hexch1 = (float *) malloc (sizeof(float)*nElt2*nElt1*(nstruct/skip));
    
         for(int i=0;i<(nstruct/skip);i++)
         {
           for(int j=0; j<nElt2; j++)//donor oxygen
           {
	     for(int k=0; k<nElt2; k++)//acceptor oxygen
             {
	       Hexch[k+j*nElt2+i*nElt2*nElt2]=0.0;
	     }
...
int **initHBcol,**initHBcol1;// storing the location of the H bond in the donor/acceptor , acceptor-H matrix 

initHBcol = (int **) malloc (sizeof(int *)*nstruct/skip);
initHBcol1 = (int **) malloc (sizeof(int *)*nstruct/skip);
     for(int i=0; i<nstruct/skip; i++)// for each frame
     {
     initHBcol[i] = (int *) malloc (sizeof(int)*initHBnum[i]);
	...
	if( Hbondxyz[j*9+i*nElt1*9+4] !=0 && exch1[j+i*nElt1] == whichwater ) 
        {
	  Hexch[Hbondneigh[j*OHsamples+i*nElt1*OHsamples+1]+Hbondneigh[j*OHsamples+i*nElt1*OHsamples]*nElt2+i*nElt2*nElt2]=1.0;
	  initHBcol[i][chker] = (Hbondneigh[j*OHsamples+i*nElt1*OHsamples+1]+(Hbondneigh[j*OHsamples+i*nElt1*OHsamples]*nElt2)); 
  ...
  lifetime
    ...
    Hexchplot.close(); */
cout<<" Done !!!"<<endl;     
//Here I am going to count, how many h bonds each water molecule has. 
//This is tricky here since it only works for a specific region. 
//However, we can compute for all atoms for checking. 
//Now I have to simply add all the numbers in every row of the nElt2*nElt2 matric for every frame and average. I can do this!!
cout <<"computing H bonds per water - see HperO.data";

float *HperO;

HperO=(float *) malloc (sizeof(float)*nElt2);
...
HperO[i]=0.0;
         ...
         //for(int i=100;i<101;i++)
         {
	       ...
	       if(j!=k)
	       {
               HperO[j]+=Hexch[k+j*nElt2+i*nElt2*nElt2];
               HperO[j]+=Hexch[j+k*nElt2+i*nElt2*nElt2];
	       }
             }
           }
         }
...
for(int i =0; i<nElt2;i++)
{
HperO[i]/=float(nstruct/skip);
}

ofstream HperOdat;

HperOdat.open("HperO.data");
for(int i =0; i<nElt2;i++)
{
HperOdat<<i<<" "<<HperO[i]<<endl;
}
HperOdat.close();
...
////////////////////////////////////LIFETIME-PART///////////////////////////////////////////
    ...
    cout <<restarts<<" restarts from each bin will be taken"<<endl;
       ...
       SDsum=(float *) malloc (sizeof(float)*SD_store);
	...
	float *FDsum, *FDsum1;
        FDsum=(float *) malloc (sizeof(float)*SD_store);
     ...
     {      
       ...
       Hexch_sized1 = (float *) malloc (sizeof(float)*initHBnum[j+i*SD_store]*SD_store);
	     ...
	     //cout<<ja<<" "<<initHBcol[j+i*SD_store][ja]<<endl;
	       Hexch_sized[ja+ia*initHBnum[j+i*SD_store]] = Hexch[initHBcol[j+i*SD_store][ja]+(j+i*SD_store)*nElt2*nElt2+ia*nElt2*nElt2];
	       Hexch_sized1[ja+ia*initHBnum[j+i*SD_store]] = Hexch1[initHBcol1[j+i*SD_store][ja]+(j+i*SD_store)*nElt2*nElt1+ia*nElt2*nElt1];
	   }
	 }
     
     
     if(transient_HB_definition == "yes")
     {
//transient time exclusion - be very careful with the inner loop here.....
      int ret=0;
           for(int ja=0; ja<initHBnum[j+i*SD_store]; ja++)//donor
           {  
            for(int ia=1;ia<SD_store-(200/timestep);ia++)
             {
               if(Hexch_sized[ja+ia*initHBnum[j+i*SD_store]]==0)
               {
                ret=1;
                for(int ka=1;ka<(200/timestep);ka++)
                {
                  if(Hexch_sized[ja+(ia+ka)*initHBnum[j+i*SD_store]]==0) ret++;
                  else if(Hexch_sized[ja+(ia+ka)*initHBnum[j+i*SD_store]]==1)
                  {
                   for(int kas=0;kas<ret;kas++)
                   {
                    Hexch_sized[ja+(ia+kas)*initHBnum[j+i*SD_store]]=1;
                   }
                   break;
                  }
                }
      ...
      /*   Hexchplot.open("Hexch.data");
	   ...
	   Hexchplot <<ia<<" ";
           for(int ja=0; ja<initHBnum[j+i*SD_store]; ja++)
             {
	       Hexchplot << Hexch_sized[ja+ia*initHBnum[j+i*SD_store]];
	     }
	      Hexchplot <<endl;
	   }
    ...
    Hexchplot.close();
	 ...
	 for(int ja=0; ja<initHBnum[j+i*SD_store]; ja++)
	 {
      ...
      cudaMalloc((void **)&dev_SD,sizeof(float)*initHBnum[j+i*SD_store]*SD_store);
      
      cudaMemcpy(dev_A,Hexch_sized,sizeof(float)*initHBnum[j+i*SD_store]*SD_store,cudaMemcpyHostToDevice);    
      cudaMemcpy(dev_SD,SD,sizeof(float)*initHBnum[j+i*SD_store]*SD_store,cudaMemcpyHostToDevice);
      
     // cout << "Memory required to store Hexch information: "<<initHBnum[j+i*SD_store]*SD_store*4/float(1000000000)<< " Gbs " << endl;
     // cout << "Memory for local HBAF storage: "<<(initHBnum*SD_store*restarts)*4/float(1000000000)<< " Gbs" << endl;
      ...
      HBAF_calc<<<dimGrida,dimBlocka>>>(dev_A,dev_A,dev_SD,SD_store,initHBnum[j+i*SD_store],i,j,origins,skips);
      cudaMemcpy(SD,dev_SD,sizeof(float)*initHBnum[j+i*SD_store]*SD_store,cudaMemcpyDeviceToHost);
      cudaFree(dev_A);
      ...
      HBAF_calc<<<dimGrida,dimBlocka>>>(dev_A,dev_A,dev_SD1,SD_store,initHBnum[j+i*SD_store],i,j,origins,skips);
...
*/
	...
	FD_avg1[ja]/=avg_every;
     ...
     float lifetime,lifetime1;
     float *int_SDsum,*int_SDsum1;
     float *cum_SDsum,*cum_SDsum1;
     ...
     for(int ja =1; ja<SD_store;ja++)
      {
       ...
       if(ja == SD_store-1) {lifetime = cum_SDsum[ja];lifetime1 = cum_SDsum1[ja];}
      }  
	...
	for(int k=0; k<nElt2; k++)//acceptor
	{
	  Hbondnum[i]+=Hexch[k+j*nElt2+i*nElt2*nElt2];
	}
      }
