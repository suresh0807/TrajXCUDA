...
MSDcounter = (int *) malloc (sizeof(int)*MSDsteps);
SDavg=(float *) malloc (sizeof(float)*SD_store*MSDsteps);
    for(int j =0; j<SD_store;j++)
    {
      for(int k=0; k<MSDsteps;k++)
      {
      SDavg[k+j*MSDsteps]=0.0;
      }
    }


    for(int i=0; i<num_bins;i++) // split the whole traj into bins of SD_store length
...
cudaMemcpy(dev_inout,inout, sizeof(int)*nElt1*(origins/skips),cudaMemcpyHostToDevice);
      for(int j=1; j<SD_store; j++) //We do MSD calculation for all atoms and for all origins for 1 step at the same time!!!
      {
      //cudaMemcpy(dev_inout,inout, sizeof(int)*nElt1*(origins/skips),cudaMemcpyHostToDevice);
...
/*
    for(int i1=0;i1<origins/skips;i1++)
    {
     for(int j1=1;j1<SD_store;j1++)
     {
      for(int k1=0; k1<nElt1; k1++)
      {
       SD[k1+j1*nElt1+i1*nElt1*SD_store]/=float(numc[j1+i1*SD_store]); // average over nElt1 - perfectly normal.
      }
     }
    }
  */
    ...
    cout << (i+1)*diffuse_time << " pico seconds done"<<endl;  
     ...
     SDavgf=(float *) malloc (sizeof(float)*SD_store*MSDsteps);
     
    for(int i1=0; i1<SD_store; i1++)
    {
      for(int j1 =0; j1<MSDsteps;j1++)
      {
        SDavgf[j1+i1*MSDsteps]=0.0;
      }
    }
	...
	for(int j1=1;j1<SD_store;j1++)
       {
       SDavgf[MSDchaos[k1+i1*nElt1*skips+i*origins*nElt1]+j1*MSDsteps] += SD[k1+j1*nElt1+i1*nElt1*SD_store];
       }
       MSDcounter[MSDchaos[k1+i1*nElt1*skips+i*origins*nElt1]]++;
     }
    }
     ...
     for(int i1=0; i1<SD_store; i1++)
    {
      for(int j1 =0; j1<MSDsteps;j1++)
      {
        SDavgf[j1+i1*MSDsteps]/=MSDcounter[j1];
	SDavg[j1+i1*MSDsteps]+=SDavgf[j1+i1*MSDsteps];
      }
    }
...
//      cudaMemcpy(SD,dev_SD,sizeof(float)*nElt1*SD_store*origins,cudaMemcpyDeviceToHost);
  ...
  /*   
    ...
    cudaMalloc((void **)&dev_SDavg,sizeof(float)*SD_store*MSDsteps);
    cudaMalloc((void **)&dev_MSDchaos,sizeof(float)*nElt1);
    cudaMemcpy(dev_SDavg,SDavg,sizeof(float)*SD_store*MSDsteps,cudaMemcpyHostToDevice);
    cudaMemcpy(dev_MSDchaos,MSDchaos,sizeof(float)*nElt1,cudaMemcpyHostToDevice);
    dim3 dimBlocks(1024,1,1);
    dim3 dimGrids((SD_store+dimBlocks.x-1)/dimBlocks.x,1,1);

    SDreduce<<<dimGrids,dimBlocks>>>(dev_SDavgf,dev_SDavg,SD_store,nElt1,dev_MSDchaos,MSDsteps);
    cudaMemcpy(SDavg,dev_SDavg,sizeof(float)*SD_store*MSDsteps,cudaMemcpyDeviceToHost);
     
    
    cudaFree(dev_SDavgf);
    cudaFree(dev_SDavg);
    cudaFree(dev_MSDchaos);
 /*
    if(msd_for == "int")
    {
...
// printing the msd data to be visualized
    ...
    FILE *MSDplot=fopen("msd.data","wt");
      fprintf(MSDplot,"# Time (ps) MSD (A^(2)) %f\n",MSDint);
    for(int i =1; i<SD_store;i++)
    {
      fprintf(MSDplot,"%f ",(float(i)*timestep)/1000.0);
      for(int j =0; j<MSDsteps;j++)
      {
      //fprintf(MSDplot,"%f ",SDavg[j+i*MSDsteps]/(num_bins*counter[j]));
      fprintf(MSDplot,"%f ",SDavg[j+i*MSDsteps]/(num_bins));
      }
      fprintf(MSDplot,"\n ");
    }
    fclose(MSDplot);
    ...
    for(int i =1; i<SD_store;i++)
    {
      fprintf(DIFFplot,"%f ",(float(i)*timestep)/1000.0);
      for(int j =0; j<MSDsteps;j++)
      {
      fprintf(DIFFplot,"%f ",(SDavg[j+i*MSDsteps]/(num_bins*DOF*i*timestep))*10000);
      }
      fprintf(DIFFplot,"\n ");
    }
    fclose(DIFFplot);
